...
<p align="center">A 403 KB Mac app that keeps <b>Claude Desktop</b> working after you close the lid.</p>
...
<p align="center">
  <a href="../../releases/latest"><img alt="Download" src="https://img.shields.io/badge/download-Awake%201.1.0-1E6FE0?style=flat-square"></a>
...
Awake asks for **one** thing, and nothing else.
...
### Optional: *Stop asking for my password*

Off by default. Turning it on in Options installs a single file, `/etc/sudoers.d/awake`, holding exactly two permitted command lines:
...
That is the entire grant. The arguments are fixed, so it cannot be steered into running anything else as root, it applies only to the user who installed it, and it leaves **no daemon or helper process** behind. The file is generated by Awake, checked with `visudo` as root before it is put in place, and written `root:wheel 0440` — a file that does not parse never becomes part of your sudoers set.

Turn the switch off to remove it, or by hand:
...
1. Download `Awake-1.1.0.dmg` from [Releases](../../releases/latest) and drag Awake to Applications.
...
- **403 KB** universal binary, no frameworks bundled, no dependencies.
